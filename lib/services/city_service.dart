...
class CityService {
  ...
  Future<List<CityModel>> getCityModels() async {
    ...
    try {
      response = response = await get(Uri.parse(apiUrl.city));
  ...
  Future<CityModel> getCityById(String id) async {
    ...
    CityModel result;
    try {
      response = await get(Uri.parse('${apiUrl.city}/$id'));
  ...
  Future<CityModel> createCity(CityModel cityModel) async {
    ...
    CityModel result;
    try {
      response = await post(Uri.parse(apiUrl.city),
  ...
  Future<bool> deleteCity(String id) async {
    ...
    bool result = false;
    try {
      response = await delete(Uri.parse('${apiUrl.city}/$id'));
      if (response.statusCode == 204) {
        result = true;
      }
    } catch (Exception) {
