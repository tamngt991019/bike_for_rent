...
class RoleService {
  ...
  Future<List<RoleModel>> getRoleModels() async {
    ...
    try {
      response = response = await get(Uri.parse(apiUrl.role));
  ...
  Future<RoleModel> getRoleById(String id) async {
    ...
    RoleModel result;
    try {
      response = await get(Uri.parse('${apiUrl.role}/$id'));
  ...
  Future<RoleModel> createRole(RoleModel roleModel) async {
    ...
    RoleModel result;
    try {
      response = await post(Uri.parse(apiUrl.role),
  ...
  Future<bool> deleteRole(String id) async {
    ...
    bool result = false;
    try {
      response = await delete(Uri.parse('${apiUrl.role}/$id'));
      if (response.statusCode == 204) {
        result = true;
      }
    } catch (Exception) {
